    ...
    if (glcontext->es) {
        ...
        while (*extensions) {
            ...
            for (i = 0; i < nb_extensions; i++) {
                ...
                if (!extension)
                    break;
                if (!strcmp(*extensions, extension)) {
                    found = 1;
                    break;
                }
            }
...
static int glcontext_probe_extensions(struct glcontext *glcontext)
{
    ...
    for (int i = 0; i < NGLI_ARRAY_NB(glfeatures); i++) {
        ...
        const size_t *funcs_offsets = glfeature->funcs_offsets;
        while (*funcs_offsets != -1) {
            if (!*(void **)((uint8_t *)gl + *funcs_offsets))
                continue;
            funcs_offsets++;
        }
    ...
    ngli_glGetIntegerv(gl,
                       GL_MAX_TEXTURE_IMAGE_UNITS,
                       &glcontext->max_texture_image_units);
...
int ngli_glcontext_load_extensions(struct glcontext *glcontext)
{
    ...
    int ret = glcontext_probe_extensions(glcontext);
    if (ret < 0)
        return ret;
